...
var currentPage = 1;
var pagelimit = 10;
var lastPage = Math.ceil(data.length / pagelimit);

function generateButtons() {
    let buttons = [];
    let start = currentPage-2>0 ? currentPage-2 : 1;
    let end = currentPage+2>lastPage ? lastPage : currentPage+2;
    // let start = currentPage;
    // let end = lastPage
    for (let i = start; i <= end; i++) {
        let eachbuttton = `<button onclick="generatePage(${i})" class="${currentPage === i ? 'active' : ''}">${i}</button>`;
        buttons.push(eachbuttton);
    }
    console.log(buttons);
    document.getElementById('dynamicbuttons').innerHTML = buttons.join('')
}
function generateBody(ObjArray) {
    ('#demo').pagination({
        dataSource: data,
        pageSize: 10,
        showPrevious: false,
...
function generateHead(singleObj) {
    let keys = Object.keys(singleObj);
    console.log(keys)
    let displayHead = '';
    for (let i = 0; i < keys.length; i++) {
        displayHead += '<th>' + keys[i] + '</th>';
    }
    console.log(displayHead);
    document.getElementById('heading').innerHTML = displayHead;
}
function generateRow(singleObj) {
    let keys = Object.keys(singleObj);
    console.log(keys)
    let displayHead = '';
    for (let i = 0; i < keys.length; i++) {
        displayHead += '<td>' + singleObj[keys[i]] + '</td>';
    }
    console.log(displayHead);
    return displayHead;
}
...
function generateBody(ObjArray) {
    ...
    for (let i = 0; i < ObjArray.length; i++) {
        displayBody += '<tr>' + generateRow(ObjArray[i]) + '</tr>';
    }
    document.getElementById('data').innerHTML = displayBody;
